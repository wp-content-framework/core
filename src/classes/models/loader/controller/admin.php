<?php
/**
 * WP_Framework Classes Models Loader Controller Admin
 *
 * @version 0.0.1
...
namespace WP_Framework\Classes\Models\Loader\Controller;
...
/**
 * Class Admin
 * @package WP_Framework\Classes\Models\Loader\Controller
 * @property-read \WP_Framework\Classes\Controllers\Admin\Base $page
 */
class Admin implements \WP_Framework\Interfaces\Loader, \WP_Framework\Interfaces\Nonce {

	use \WP_Framework\Traits\Loader, \WP_Framework\Traits\Nonce;
	...
	/**
	 * @var \WP_Framework\Classes\Controllers\Admin\Base[] $_pages
	 */
	private $_pages = [];
	...
	/**
	 * @return \WP_Framework\Classes\Controllers\Admin\Base|null
	 */
	private function load_page() {
		try {
			...
			if ( ! empty( $_page ) && is_string( $_page ) && preg_match( $pattern, $_page, $matches ) ) {
				...
				$instance = $this->get_class_instance( $this->get_class_setting( $page, $add_namespace ), '\WP_Framework\Classes\Controllers\Admin\Base' );
				if ( false !== $instance ) {
					/** @var \WP_Framework\Classes\Controllers\Admin\Base $instance */
					$this->do_action( 'pre_load_admin_page', $instance );

					return $instance;
				}
				$this->app->log( sprintf( $this->app->translate( '%s not found.' ), $_page ), [
					'$_GET[\'page\']' => $_page,
					'$page'           => $page,
	...
	private function sort_menu() {
		...
		$pages = $this->app->utility->array_map( $this->_pages, function ( $p ) {
			/** @var \WP_Framework\Classes\Controllers\Admin\Base $p */
			return $this->get_page_prefix() . $p->get_page_slug();
		} );
		$pages = array_combine( $pages, $this->_pages );

		/** @var \WP_Framework\Classes\Models\Custom_Post $custom_post */
		$custom_post = \WP_Framework\Classes\Models\Custom_Post::get_instance( $this->app );
		$types       = $custom_post->get_custom_posts();
		$types       = array_combine( $this->app->utility->array_map( $types, function ( $p ) {
			/** @var \WP_Framework\Interfaces\Helper\Custom_Post $p */
	...
	protected function get_instanceof() {
		return '\WP_Framework\Classes\Controllers\Admin\Base';
	}
