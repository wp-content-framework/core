<?php
/**
 * WP_Framework Classes Models Loader Controller Api
 *
 * @version 0.0.1
 ...
 */
class Api implements \WP_Framework\Interfaces\Loader, \WP_Framework\Interfaces\Nonce {
	...
	private function register_rest_api() {
		...
		foreach ( $this->get_api_controllers() as $api ) {
			/** @var \WP_Framework\Classes\Controllers\Api\Base $api */
	...
	private function register_script_common( callable $get_view_params ) {
		...
		/** @var \WP_Framework\Traits\Controller\Api $api */
		foreach ( $this->get_api_controllers() as $api ) {
	...
	/**
	 * @param \WP_Framework\Classes\Controllers\Api\Base $api
	 */
	private function ajax_action_execute( \WP_Framework\Classes\Controllers\Api\Base $api ) {
	 ...
	 */
	private function get_ajax_action_result( \WP_Framework\Classes\Controllers\Api\Base $api ) {
		if ( ! $this->nonce_check() ) {
	...
	protected function get_instanceof() {
		return '\WP_Framework\Classes\Controllers\Api\Base';
	}
	...
	private function get_api_controllers() {
		...
		if ( ! $this->app->utility->doing_ajax() ) {
			/** @var \WP_Framework\Traits\Controller\Api $class */
